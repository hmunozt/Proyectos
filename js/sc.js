...
function updateHtml(){
    $.each(data, function(key, val) {
        ...
        if (key == 'device15'){
            if (val !== '' && val !== null){
                if (availableVideos.indexOf(convertToUpperMP4(val)) != -1){
                    $("#rigt_video").attr('src','video/'+convertToUpperMP4(val));
                }
                //else{
                //    $("#rigt_video").attr('src','video/SCREEN_SAVER1.MP4');
                //}
            }
            //else{
            //    $("#rigt_video").attr('src','//:0');
            //}
        }
